...
class Tables extends Admin_Controller 
{
	...
	public function fetchTableData()
	{
		...
		foreach ($data as $key => $value) {
			...
			$available = ($value['available'] == 1) ? '<span class="label label-success">Đang trống</span>' : '<span class="label label-warning">Đã đặt</span>';
	...
	public function update($id)
	{
		...
		if($id) {
			...
			$this->form_validation->set_rules('edit_available', 'Available', 'trim|required');
	        ...
	        if ($this->form_validation->run() == TRUE) {
	        	$data = array(
        			...
        			'available' => $this->input->post('edit_available'),
	...
	public function remove()
	{
		...
		if($table_id) {
			...
			if($delete == true) {
				$response['success'] = true;
				$response['messages'] = "Đã xoá thành công";	
			}
			else {
				$response['success'] = false;
				$response['messages'] = "Lỗi";
